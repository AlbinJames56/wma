                ...
                <Container className=" p-3 rounded bg-white border shadow my-5 ">
                            ...
                            {event.tickets.map((ticket, index) => (
                                ...
                                >
                                    <div>
                                        ...
                                        <p>
                                            {findMinAmount(ticket.categories)}$ - {findMaxAmount(ticket.categories)}$
                                        </p>
                                    </div>
                                    <Button variant="outline-dark" id="rg-4-3" onClick={() => handleTicketTypeSelect(index)}>Select</Button>
                                </Card.Body>
                            ))}
                        </Card>
                    )}
                    ...
                    {ticketType !== null && page === 1 && (
                        <Card className="rg1-5 mt-3 bg-white rounded p-3">
                            <Card.Title style={{ padding: '10px' }}>Select Tickets For</Card.Title>
                            <h4 className="text-center rg2-1">{event.tickets[ticketType].name}</h4>
                            {event.tickets[ticketType].categories.map((pricing, index) => (
                                <Row
                                    ...
                                    <Col xs={2} className="d-flex align-items-center" id="rg2-3">
                                        ...
                                        />
                                        <span className="mx-2">{data.ticketCount[pricing.name]}</span>
                                        <AddCircleOutline
                                            onClick={() => {
                                                setWarning(false);
                                                setData({
                            ...
                            <div className="d-flex justify-content-between mt-3 rg2-4">
                                <Button variant="dark" onClick={setBackPage}>Back</Button>
                                <Button variant="primary" onClick={handleTicketSelect}>Next</Button>
                            </div>
                        </Card>
                    )}

                    {page === 2 && (
                        <EventRegDetails event={event} registrationData={data} setBackPage={handleTicketTypeSelect} />
                    )}
                </Container></div>
            )}
        </div>
